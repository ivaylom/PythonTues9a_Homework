...
def onTimer():
    ...
    if seconds <= 0:
        ...
        if counter >= 40:
            tkinter.messagebox.showinfo( "Game over!","You played very well! Your score is %s!" % counter)
        elif counter < 40 and counter >= 30:
            tkinter.messagebox.showinfo( "Game over!","Your score is %s" % counter)
        else:
            tkinter.messagebox.showinfo( "Game over!","You can do better! Your score is %s!" % counter)
    else:
        button.after(1000, onTimer)
window.mainloop()

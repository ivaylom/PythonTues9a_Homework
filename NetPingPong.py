...
def onTimer():
    ...
    barCoords = c.coords(bar)
    byteArrayDot = pickle.dumps(dotCoords)
    byteArrayBar = pickle.dumps(barCoords)
    connection.send(byteArrayDot)
    connection.send(byteArrayBar)
...
def waitNetwork():
  while True:
    bArrayDot = connection.recv(1000)
    bArrayBar = connection.recv(1000)
    dotCoords = pickle.loads(bArrayDot)
    barCoords = pickle.loads(bArrayBar)
